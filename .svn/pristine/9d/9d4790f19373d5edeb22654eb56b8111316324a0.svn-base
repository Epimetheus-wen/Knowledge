	...
	<view class="previewWeb" @tap="dowInfo">
		...
		<view class="previewWeb_main" :style="'min-height: calc(100vh - ('+((staHeight*2)+80)+'rpx) - 190rpx);'">
            ...
            <view class="info">
                ...
                <button :style="zipShow?'display:none;':'display:block;'" v-else class="again" @tap="getFilePath">{{againTxt}}</button>
                <view v-if="zipShow" class="zipShow">请前往PC端操作</view>
            </view>
		</view>
        <view v-if="loginFX" class="footer_but">
	...
	export default {
        ...
        onShareAppMessage(res) {
            if(this.obj.scope==2){
                ...
                let objA=this.obj
                objA.filePath=this.filePath
                let str=JSON.stringify(objA)
                return {
                    title: this.obj.accessory_name,
                    // path: '/pages/outside/share?str='+encodeURIComponent(str),
                    // path: '/pages/outside/share?str='+encodeURIComponent(str),
                    path: '/pages/preview/preview?str='+encodeURIComponent(str),
                    imageUrl:this.haibaoUrl,
                    success: function(ress) {
                        console.log(ress)
                    }
		...
		methods: {
            ...
            share() {
                ...
                if(this.imgIcon=="../../static/home/img.png"||this.imgIcon=="../../static/home/zip.png"){
                    codecanvas.drawImage(that.imgIcon, (93*that.rpx), (30*that.rpx), (40*that.rpx), (40*that.rpx))
                }else{
                    codecanvas.drawImage(that.imgIcon, (93*that.rpx), (30*that.rpx), (40*that.rpx), (48*that.rpx))
                }
            ...
            getFilePath(){
                ...
                wx.getStorage({
                    ...
                    success(res) {
                        if(res.data){
                            that.$post("/apiIndex.html?f=getFilePath",{"id":that.obj.id})
                            .then((res) => {
                                console.log("getFilePath",res)
                                that.downloadFile(res.data)
                                that.filePath=res.data
                            })
                        }else{
                            that.downloadFile(that.obj.filePath)
                        }
                    },
                    fail(res) {
                        that.downloadFile(that.obj.filePath)
                ...
                // this.$post("/apiIndex.html?f=getFilePath",{"id":this.obj.id})
                //     .then((res) => {
                //         console.log("getFilePath",res)
